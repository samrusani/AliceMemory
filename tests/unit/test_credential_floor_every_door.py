...
def test_door4_provenance_is_read_with_its_keys() -> None:
    """Provenance is read with its keys.

    A bare key segment is a secret name only with a secret qualifier, so an
    OpenClaw ``openclaw_dedupe_key`` over a digest, and an ordinary quote, are
    not refused. An opaque value under ``api_key`` is. A credential value
    under a name that is not a secret name is still caught.
    """

    digest = hashlib.sha256(b"workspace").hexdigest()
    keyed_secret = {"api_key": "Xq9mZt2L" + "xP9wKc4BVq7m"}
    ordinary = {
        "quote": "Deploys go out on Tuesdays. The memory_key is project.alice.status.",
        "openclaw_dedupe_key": digest,
        "source_kind": "openclaw_import",
    }
    assert carries_credential_material(keyed_secret)
    assert not carries_credential_material(ordinary)
    assert carries_credential_material({"quote": f"deployed with {PAT}"})

    for provenance in (ordinary, {"openclaw_dedupe_key": digest, "source_kind": "openclaw_import"}):
        store = _StoredContinuityObject()
        with pytest.raises(LookupError):
            _correct(
                store,
    ...
    for provenance in (keyed_secret, {"quote": f"deployed with {PAT}"}):
        store = _StoredContinuityObject()
        with pytest.raises(ContinuityReviewValidationError, match="credential material"):
            _correct(
                store,
                ContinuityCorrectionInput(
                    action="edit", body={"decision_text": "keep it"}, provenance=provenance
                ),
            )
        assert store.writes == []
        sink = _RecordingContinuityStore()
...
def test_round2_import_reads_the_value_and_metadata_as_mappings(tmp_path, capsys) -> None:
    """Import door. The value column and metadata_json are both read with
    their keys. A bare key segment is a secret name only with a secret
    qualifier, so fact_key and memory_key import, and api_key does not.
    A credential value under a name that is not a secret name is still caught."""
    ...
    err = capsys.readouterr().err
    assert code == 1 and f"memory {memory_id} carries credential material (value)" in err
    code, memory_id = _import_one_memory(tmp_path / "meta", metadata_json={"api_key": "Xq9mZt2L" + "xP9wKc4BVq7mZt2"})
    err = capsys.readouterr().err
    assert code == 1 and f"memory {memory_id} carries credential material (metadata_json)" in err
    # Guards the guard: names that are not secret names import, including an
    # opaque value under stripe_key.
    code, _memory_id = _import_one_memory(
        tmp_path / "clean",
        value={"text": "notes", "fact_key": "release_mode", "memory_key": "project.alice.status"},
        metadata_json={"dedupe_key": HEX64, "stripe_key": "Xq9mZt2L" + "xP9wKc4BVq7mZt2"},
    )
