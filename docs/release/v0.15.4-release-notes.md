...
return a generic code and no detail.** The server reports `tool_not_found`,
`tool_request_failed`, or `tool_execution_failed`. The message does not say
why, so an agent cannot distinguish a bad argument from a missing record
from an unbootstrapped database. The server log carries the real reason.
