...
- provenance is checked with its keys. A routing `session_key` of the form `agent:<profile>:<channel>:<kind>:<digits>` is still imported. A secret name over an opaque value is skipped
