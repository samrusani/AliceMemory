...
- no automatic migration to Postgres; `alice-memory export` creates a versioned, integrity-checked local backup and `alice-memory import` restores it into another local database (portable ids and timestamps preserved). `--quarantine` is the owner's recovery path when that backup holds a credential and the source vault is gone. It stores the named memory as `rejected` and replaces its text with `[quarantined on import]`. It does not import the credential, and it does not rewrite source text, provenance quotes, open-loop titles, or graph explanations. It is not a PostgreSQL import.
