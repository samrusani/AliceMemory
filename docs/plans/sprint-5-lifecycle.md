...
1. Retry loop (live harm, first). `_capture_worker` retries a failed POST by starting a new worker at once, with no backoff, in the plugin's `__init__.py`. One sync turn against a server returning 422 made 14,913 POSTs in 0.5 seconds. Every Hermes user who turned on sync-turn capture hammers their local server until the session ends. Fix: capped exponential backoff, a per-item attempt limit, then drop and count the item. Test: against a stub that always fails, attempts in one second stay at or under the limit. Mutation: remove the backoff.
