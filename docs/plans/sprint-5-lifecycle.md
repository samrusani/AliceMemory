...
1. Claude Code has no `SessionEnd` or `Stop` hook in this repo. `SessionEnd` is not in Python. Outside this note it appears once, in `docs/roadmap-friction-first.md`, in a claude-mem list. `Stop` is ordinary prose in Python (`vnext_brain.py`, scheduler help in `cli/parser.py`) and in other docs. It is not a registered hook. `transcript_path` appears only in this note. The installer does not register `SessionEnd` or `Stop`. The queue says the host emits `SessionEnd` and `Stop` and includes `transcript_path`.
   ...
   - Register both `Stop` and `SessionEnd`. Cost: this repo does not register `Stop`, so this option would add that hook. If the host fires it on each turn, a long session can give that user eight distinct `source_id`s of per-turn proposals. Later runs for that user then add nothing. Rows for another `user_id` do not count. The user reviews turn scraps instead of one session note.
...
2. Hermes already has an end callback. `on_session_end` can POST `/v0/continuity/captures/commit` for a queued sync-turn when the capture worker is not still alive after the wait and bridge mode is `assist` or `auto`. `commit_continuity_captures` auto-saves only some candidates and writes continuity objects, not a memory through `create_memory`. The queue says Hermes has no end hook.
   ...
   - Leave `on_session_end` alone. Hermes proposals are the sleep pass. Cost: a user who turned on sync-turn capture can still reach that endpoint at session end, under the wait and mode rules above. In `assist` mode an auto-save needs an allowlisted type, `explicit`, and confidence at least 0.9. In `auto` mode an allowlisted type auto-saves at confidence at least 0.85, and `explicit` is not required. `_resolve_commit_decision` returns `no_op` when `candidate_type` is `no_op`, before either mode branch. An ack-only turn is that `no_op` (`confidence` 1.0, `explicit` false). The commit record has `continuity_object: None` and does not insert a row. A repeat of a stored fingerprint is `duplicate_noop`. `queued_for_review` is only the remaining case, including a fresh `note`. A queued or auto-saved result is a continuity object, not a sidecar proposal. Review still gates that remaining case.
...
5. `run_local_vault_sleep` stops adding when the caller's distinct `source_id`s already in the file, plus rows this run is about to add, reach `SLEEP_PROPOSAL_CAP` (8). Rows for another `user_id` stay in the file and do not count, so a second user can append another eight. Accepting does not remove a row: `alice_memory_commit` does not edit the sidecar. A later sleep run still writes when that user has fewer than eight distinct `source_id`s, including when the file already holds eight rows for someone else.

   - Leave that as it is. Cost: once that user's distinct `source_id`s in the file reach eight, later sleep runs for that user add nothing. Committing them does not free a slot, because `alice_memory_commit` does not edit the sidecar. A second user can still append another eight. Reviewing the first user's rows leaves that user's cap in place.
