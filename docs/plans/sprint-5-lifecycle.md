...
2. The sleep writer applies the commit door's credential check.
...
- Sources are listed oldest first (`ORDER BY captured_at ASC, id ASC`). The brief shows the newest. In the team's probe, the one source the brief showed was never proposed. The ruling keeps oldest-first and says to document that order.
...
The sidecar is a persistence path. The sleep writer applies exactly the commit door's check.

Today `run_local_vault_sleep` does not. It copies chunk text into the row and calls `_write_jsonl` with no check. That is a real gap.
...
Ruling: factor the body of `_request_contains_secret_marker` into one helper that refuses when `credential_verdict` refuses or when `legacy_credential_check.commit_gate_refuses` refuses. The commit door and `run_local_vault_sleep` both call it.

- New rows. Run the helper on the 160-character excerpt and on the full first chunk it is cut from. Drop the row if either refuses. A withheld source takes no cap slot and is checked again on every run. The chunk matters. When the check runs on the cut excerpt alone, `AKIA` plus up to 11 characters of an AWS-shaped key id passes, and so does `PASSWORD_DB=` plus up to 5 value characters. The full chunk was refused every time. Measured cost: all 259 tracked markdown files in the repo, imported as a vault, gave 0 refusals. If measurement on real exports finds the chunk rule withholding an excerpt whose chunk holds no credential shape, narrow it: check the excerpt extended to the end of the token the cut falls in.
- Existing rows. Before any rewrite, check the caller's rows (excerpt, and the source's first chunk if the source still exists) and other users' rows (excerpt), and drop refused rows. Rewrite only when a row was written or dropped. Otherwise leave the file byte-identical.
- File mode. Remove a stale `.tmp`, create the new one with `O_CREAT|O_EXCL` and mode 0600, then replace. If the sidecar exists with group or other bits, chmod it to 0600 even when nothing is rewritten. Today it is written 0644 beside a 0600 `memory.db`.
...
- The `.tmp` bytes captured at the replace call never hold a refused value.
- Mode 0600 after a writing run, after a run with a stale 0644 `.tmp`, and after a no-op run on an existing 0644 sidecar.
- Rollback: a sidecar this branch wrote loads and runs under v0.16.0's `load_sleep_proposals` and sleep.
...
- Order. Sleep proposes the oldest sources first, while the brief shows the newest. Document that order. In the team's probe, the one source the brief showed was never proposed.
...
1. Retry loop (live harm, first). `_capture_worker` retries a failed POST by starting a new worker at once, with no backoff, in the plugin's `__init__.py`. One sync turn against a server returning 422 made 14,913 POSTs in 0.5 seconds. Every Hermes user who turned on sync-turn capture hammers their local server, and on main it can keep going after the session ends. Fix: capped exponential backoff, a per-item attempt limit, then drop and count the item. Test: against a stub that always fails, attempts in one second stay at or under the limit. Mutation: remove the backoff.
2. Credential fallback. When `POST /v0/continuity/captures/commit` refuses a turn with HTTP 400 for credential material, `_post_capture` re-sends the raw "User: ... Assistant: ..." text to `POST /v0/continuity/captures`. That route runs no credential check and stores the token in a triage capture event that the list route returns. Fix: never fall back on a 400. Test: a refused commit with a runtime-built token makes no captures POST and leaves no `capture_events` row holding it. Mutation: restore the fallback. Update `docs/memory/promotion-personas.md` to say the plugin used to route one door into the other.
