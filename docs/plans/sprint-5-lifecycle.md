...
- Sleep proposes oldest first (`LIST_SOURCE_IDS_SQL`: `ORDER BY captured_at ASC, id ASC`). That is not sidecar append order. The session brief shows the newest. In the team's probe, the one source the brief showed was never proposed.
