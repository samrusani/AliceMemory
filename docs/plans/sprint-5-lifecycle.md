...
- A review surface for chat proposals. `sleep-proposals` lists this user's sleep rows. Before that command, `doctor` reported 0 waiting while 8 proposals sat in the sidecar, because `candidates waiting` counts memory rows.
...
At session end, no host in this repo has a proposal hook. The user keeps today's sleep pass. Rows the writer adds are `proposed` and are not memories. The file beside the database is not capped at eight total rows. The cap counts rows the credential check kept. A row whose source already has an active or accepted memory does not count toward `SLEEP_PROPOSAL_CAP` (8) and is not offered again. The row stays in the file. The next sleep run can write into the freed count. Rows for another `user_id` stay in the file and do not count, so a second user can append another eight. Recall and search stay as they were. A memory appears only when someone later commits the proposed text with `alice_memory_commit`.

`alice-memory sleep-proposals` lists this user's rows. `alice-memory doctor` prints `candidates waiting`, and that count is memory rows, then `sleep proposals`. Before that doctor line, it reported 0 waiting while 8 proposals sat in the sidecar. Accepting via `alice_memory_commit` does not edit the sidecar. After that commit, the next sleep run can write into the freed count.
...
- `SLEEP_PROPOSAL_CAP` is 8. The cap counts rows the credential check kept. A row whose source already has an active or accepted memory does not count toward that cap and is not offered again. The row stays in the file. The next sleep run can write into the freed count.
...
- Sleep and `sleep-proposals` order by source `captured_at` ascending, then id ascending (`LIST_SOURCE_IDS_SQL`: `ORDER BY captured_at ASC, id ASC`). That is not sidecar append order. The session brief shows the newest. In the team's probe, the one source the brief showed was never proposed.
...
The commit door is `_request_contains_secret_marker` in `vnext_memory_commit.py`. It returns `commit_door_secret_verdict(...)`. The sleep writer and the listing call that helper through `_text_refused`. The helper is what calls `credential_verdict` and then `commit_gate_refuses` in `legacy_credential_check.py`. `legacy_commit_gate_refuses` is only the import alias in `vnext_memory_commit.py` (`from alicebot_api.legacy_credential_check import commit_gate_refuses as legacy_commit_gate_refuses`). It is not the function. The earlier note named the alias as if it were the function. `legacy_credential_check`'s docstring names the sleep writer and `create_continuity_object_record`, and says every remaining door keeps the floor alone.

Ruling: factor the body of `_request_contains_secret_marker` into one helper that refuses when `credential_verdict` refuses or when `legacy_credential_check.commit_gate_refuses` refuses. The commit door returns `commit_door_secret_verdict`. `run_local_vault_sleep` and the listing call it through `_text_refused`. `create_continuity_object_record` calls it on the title and on the body's string values.
...
- Docstring. `legacy_credential_check`'s docstring names the sleep writer and `create_continuity_object_record`. Both call `commit_door_secret_verdict`. Every remaining door keeps the floor alone.
...
This design does not delete or rewrite the sidecar row when the commit succeeds. The row stays `proposed`. Accepting via `alice_memory_commit` does not edit the sidecar. After that commit, the next sleep run can write into the freed count.

## Review surface and the sticky cap

Ruling: the cap is 8. It counts rows the credential check kept. A row whose source already has an active or accepted memory does not count toward that cap and is not offered again. The row stays in the file. The next sleep run can write into the freed count. `sleep-proposals` lists this user's rows. The earlier plan kept the review surface out of scope.

The receipt always prints six lines: proposals written, already present, skipped as already linked, cap, sources withheld, and existing rows removed. When the caller's counting rows are full and at least one source was not proposed because of the cap, the receipt adds two lines. One gives the number of sources not proposed. The other says sleep adds no proposals for this user until a counting row is removed from the sidecar or its source has an active or accepted memory, and gives the sidecar's path. After a source has an active or accepted memory, the next sleep run can write into the freed count.

The review surface is one PR, CLI only, no fourth MCP tool.

- A listing command that prints each of the caller's proposals framed and JSON-quoted, with its source id and the exact `alice_memory_commit` arguments to accept it, including `source_refs` and the source's `domain`, `sensitivity`, and `project_scope`. The commit line is ASCII-escaped. A row whose source already has an active or accepted memory is not offered again. Rows the fence, the commit door, or that skip leaves out are counted on a `rows not shown: N` line only when that count is positive, and a listing with no proposal line is `No sleep proposals.` followed by the not-shown line when rows were also left out, while the listing is `No sleep proposals.` alone when nothing was left out. It runs the shared commit-door check again, through `_text_refused`, on the stored excerpt and on the cut window of the first chunk, and writes nothing. A clean excerpt whose chunk trips the legacy gate only past that cut is still listed. There is one `_first_chunk_text`. It is a new read path, so it must pass every control the brief applies. Test each one.
- A `doctor` line `sleep proposals`, printed after `candidates waiting`, which counts memory rows. An unreadable sidecar prints `sleep proposals: unreadable` and the other census lines stay.
...
- Order. Sleep proposes the oldest sources first, by source `captured_at` then id, and the listing uses that same order. The session brief shows the newest. In the team's probe, the one source the brief showed was never proposed.

Acceptance:

- Import 10 unlinked notes. Run 1 writes 8 and prints the cap-full lines. Commit two excerpts with `source_refs`. Run 2 writes 2 into the freed count. Those two sidecar rows are unchanged. A second run with no commit still writes 0 and prints the cap-full lines.
- A second `user_id` on the same file can still add 8.
- After a commit, the next sleep run can write into the freed count. The sidecar row stays. Accepting does not edit the sidecar.
...
3. Auto-save rule. When the 422 is fixed, auto-save applies only to user-role candidates matched by an explicit prefix rule (`decision:`, `preference:`, `commitment:`, and the others in `_CANDIDATE_PREFIX_RULES`). Every regex hit and every assistant-role candidate is queued for review, in both assist and auto mode. Continuity object creation calls the shared commit-door check from the credential ruling. For: a user who types "decision: use Postgres for billing" has said to save it, much like a commit. The previous rule would auto-save assistant narration with test output attached (4 of 10 assistant turns in the probe) and inject it into the next prefetch. Against: "auto" users asked for automatic capture, and this narrows it. The path has never worked for anyone (the 422 was on every version through v0.16.0), so no one depends on the wider rule, and queued items are kept for review. `create_continuity_object_record` calls `commit_door_secret_verdict` on the title and on the body's string values, not on a JSON dump of the body. The helper runs the credential floor and then `commit_gate_refuses`. A `PASSWORD_DB=<runtime value>` decision is refused, including a quoted assignment past the 280-character title cut. An ordinary note that quotes a word is stored, and a later candidate in the same turn is still stored.
4. The 422 fix. `_rewrite_user_id_json_body` sets `request._body` on the middleware's request before `call_next`, as the browser-clip path in `main.py` does. `BaseHTTPMiddleware.call_next` ignores a replacement `Request` and replays that cache, so a header-only JSON body includes the authenticated `user_id` and `POST /v0/continuity/captures/candidates` reaches the handler. The full-app test covers `/v0` enabled and disabled. With legacy `/v0` disabled outside development and test, the same POST returns 404 and the handler does not run. The function-level test still calls `_rewrite_user_id_json_body` on its own. On v0.16.0 the header-only POST returns 422.
...
2. Hermes `on_session_end`. Ruling: do not point it at the sleep writer. Hermes session end degrades to `alice-memory sleep`. The earlier recommendation to leave the path alone is replaced: the path has live defects, and they belong in this sprint, in the order in the Hermes track. On v0.16.0 the candidates POST returns 422 because `user_id` is only in the header and the commit is never reached. The middleware now sets `request._body` to the rewritten JSON before `call_next`, so that POST reaches the handler when legacy `/v0` is enabled. With legacy `/v0` disabled outside development and test, the same POST returns 404 and the handler does not run. Before #423 the capture worker restarted after every failure, so `on_session_end` usually found a live worker, skipped the drain, and the item stayed queued. POSTs continued after session end. Since #423 a 4xx other than 408 and 429 is dropped and counted on the first failure, other failures back off and drop after 5 attempts, and session end stops the backoff. The ruling for that last pass is one attempt per queued item within the flush timeout, then discard and count whatever is still queued. On #423, `on_session_end` first joins a live prefetch thread for up to 2 seconds. That join is outside the flush deadline. It then sets one deadline, `time.monotonic() + flush_timeout`, sets the capture stop event, and joins the capture worker with the time left on that same deadline. If the worker is not alive after that join, it drains the queue with the same deadline. The drain is skipped if the worker is still alive when the join ends. A hanging POST is capped by the time left on that one deadline. It does not get a fresh flush timeout after the join. If the prefetch thread is alive, session end can take the prefetch join plus the flush timeout. The extractor is `capture_continuity_candidates`. `commit_continuity_captures` commits a candidate list it is given. The extractor sets `explicit` to true on every regex hit, so the assist-mode `explicit` test filters almost nothing. The join, drain, `no_op`, `duplicate_noop`, fresh-note, assist, auto, active, and stale facts in the session-end section stay. Neither path calls `create_memory`.
...
5. The sticky cap and the review surface. Ruling: the cap is 8. It counts rows the credential check kept. A row whose source already has an active or accepted memory does not count toward that cap and is not offered again. The row stays in the file. The next sleep run can write into the freed count. `sleep-proposals` lists this user's rows. The review surface is one CLI PR, with the listing command, the separate `doctor` line, and the slot release. `alice_memory_commit` is not touched. Accepting does not edit the sidecar. Sleep and the listing both use oldest first, by source `captured_at` then id (`ORDER BY captured_at ASC, id ASC`). The session brief shows the newest.
